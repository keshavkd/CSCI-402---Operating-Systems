...
sys_read(read_args_t *arg)
{
    ...
    if (!((err = copy_from_user(&kern_args, arg, sizeof(read_args_t))) >= 0)) {
        ...
        while (!(count <= 0)) {
            ...
            int i_read = do_read(kern_args.fd, kaddr, readlen);
            if (i_read >= 0) {
            	dbg(DBG_PRINT, "(GRADING3A)\n");
                err = copy_to_user(buff, kaddr, i_read);
                if (err >= 0) {
                	dbg(DBG_PRINT, "(GRADING3A)\n");
                    count -= i_read;
                    buff += i_read;
                    total += i_read;
                    if ((unsigned)i_read != readlen) {
                    	dbg(DBG_PRINT, "(GRADING3A)\n");
                        break;
                    }                    
                } else {
...
sys_write(write_args_t *arg)
{
    ...
    if (!((err = copy_from_user(&kern_args, arg, sizeof(write_args_t))) >= 0)) {
        ...
        while (!(count <= 0)) {
            ...
            if (err >= 0) {
            	dbg(DBG_PRINT, "(GRADING3A)\n");
                int i_write = do_write(kern_args.fd, kaddr, writelen);
                if (i_write >= 0) {
                    dbg(DBG_PRINT, "(GRADING3A)\n");
                    count -= i_write;
                    buff += i_write;
                    total += i_write;
                    if ((unsigned)i_write != writelen) {
                    	dbg(DBG_PRINT, "(GRADING3A)\n");
                        break;
                    }
                } else {
...
sys_getdents(getdents_args_t *arg)
{
    ...
    dbg(DBG_PRINT, "(GRADING3A)\n");
    while (i < maxdir) {
        int i_read = do_getdent(kern_args.fd, &dirent);
        dbg(DBG_PRINT, "(GRADING3A)\n");
        if (i_read >= 0) {
            dbg(DBG_PRINT, "(GRADING3A)\n");
            if (i_read != 0) {
